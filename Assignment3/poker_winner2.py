def type_check(a):
    res1 = int(a[2]) == int(a[1]) + 1 == int(a[0]) + 2
    res2 = int(a[2]) == int(a[1]) + 1 == int(a[0]) + 2

    # res1 = int(a[0]) == int(a[1])+1 == int(a[2])-1 or int(a[1]) == int(a[0])+1 == int(a[2])-1 or int(a[2]) == int(
    # a[0])+1 == int(a[2])-1
    # 2,1,3==1,2,1==
    # res2 = int(a[0]) == int(a[2])+1 == int(a[1])-1 or int(a[1]) == int(a[2])+1 == int(a[
    # 0])-1 or int(a[2]) == int(a[1])+1 == int(a[0])-1
    if int(a[0]) == int(a[1]) == int(a[2]):
        return 4
    elif int(a[0]) == int(a[1]) != int(a[2]) or int(a[0]) == int(a[2]) != int(a[1]) or int(a[2]) == int(a[1]) != int(
            a[0]):
        return 2
    elif res1 or res2:
        return 3
